...
export default async function CompanyDetailPage({
  ...
  const collectedSamples = samples.filter(
    (sample) => sample.collectionDate || ["collected", "testing", "completed"].includes(sample.status.trim().toLowerCase())
  );
  const collectedSampleUnits = sum(collectedSamples.map((sample) => sample.sampleCount));
  const deliveredReports = reports.filter(
    (report) =>
      Boolean(report.deliveredDate) ||
      ["delivered", "submitted", "released", "completed"].includes(report.status.trim().toLowerCase())
  );
  ...
  const activeRecurringServices = recurringServices.filter((service) => isActiveRecurring(service.status));
  const customerType =
    activeRecurringServices.length > 0
      ? "Monthly / Recurring"
      : countedWorkOrders.length > 0 || collectedSampleUnits > 0 || reports.length > 0
        ? "One-time / Existing Client"
        : quotations.length > 0 || leads.length > 0
  ...
  const timeline: TimelineEntry[] = [
    ...
    ...quotations.map((quotation) => ({
      ...
      detail: `${quotation.service} · ${formatCurrency(quotation.totalAmount)}`,
    ...
    ...workOrders.map((workOrder) => ({
      ...
      detail: `${workOrder.service} · ${formatCurrency(workOrder.totalAmount)}`,
    ...
    ...samples.map((sample) => ({
      ...
      detail: `${sample.sampleType} · ${sample.sampleCount} ${sample.sampleCount === 1 ? "sample" : "samples"}`,
    ...
    <div className="company360-page">
      ...
      <div className="company360-topbar">
        ...
        <div className="company360-topbar-copy">
          ...
          <strong>Hyderabad Operations · Client 360°</strong>
      ...
      <section className="company360-hero">
        ...
        <div className="company360-hero-main">
          ...
          <div className="company360-hero-copy">
            ...
            <span className="company360-eyebrow">Client 360° Profile</span>
      ...
      <Section eyebrow="Operations Snapshot" title="Current Client Status" description="One place for the team to understand this client's operational position." icon={<PackageCheck size={19} />}>
        <div className="company360-info-grid">
          <InfoItem icon={<BriefcaseBusiness size={17} />} label="Customer Type" value={customerType} />
          <InfoItem icon={<MapPin size={17} />} label="Locations" value={`${activeLocations} active / ${locations.length} total`} />
          <InfoItem icon={<RefreshCcw size={17} />} label="Recurring Commitment" value={recurringSamplesPerMonth > 0 ? `${recurringSamplesPerMonth} samples / month` : "Not recurring"} />
          <InfoItem icon={<TestTube2 size={17} />} label="Samples Collected" value={`${collectedSampleUnits} sample units`} />
          <InfoItem icon={<FileCheck2 size={17} />} label="Reports Submitted / Delivered" value={String(deliveredReports.length)} />
          <InfoItem icon={<Clock3 size={17} />} label="Reports Pending" value={String(pendingReports.length)} />
          <InfoItem icon={<ClipboardCheck size={17} />} label="Confirmed Business" value={`${countedWorkOrders.length} order(s) · ${formatCurrency(totalOrderValue)}`} />
          <InfoItem icon={<Banknote size={17} />} label="Payment Position" value={`${formatCurrency(amountCollected)} collected · ${formatCurrency(pendingAmount)} pending`} />
        </div>
      </Section>
        ...
        {locations.length ? (
          ...
          <div className="company360-card-grid three">
            ...
            {locations.map((location) => {
              const recurring = recurringServices.filter(
                (service) =>
                  service.locationId === location.id &&
                  isActiveRecurring(service.status)
              );

              const locationSamples = samples.filter(
                (sample) =>
                  sample.testingLocation?.trim().toLowerCase() ===
                  location.name.trim().toLowerCase()
              );

              const groupedSamples = new Map<string, number>();

              if (recurring.length) {
                recurring.forEach((service) => {
                  const type = service.sampleType.trim() || service.service.trim();
                  groupedSamples.set(
                    type,
                    (groupedSamples.get(type) || 0) + service.samplesPerMonth
                  );
                });
              } else {
                locationSamples.forEach((sample) => {
                  groupedSamples.set(
                    sample.sampleType,
                    (groupedSamples.get(sample.sampleType) || 0) + sample.sampleCount
                  );
                });
              }

              const sampleSummary = groupedSamples.size
                ? Array.from(groupedSamples.entries())
                    .map(([type, count]) => `${count} ${type}`)
                    .join(" + ")
                : "No sample collection recorded";

              const frequency = recurring.length
                ? Array.from(
                    new Set(recurring.map((service) => service.frequency))
                  ).join(", ")
                ...
                <article className="company360-record-card" key={location.id}>
                  ...
                  <div className="company360-record-body">
                    ...
                    <RecordField
                      ...
                      value={location.phone || "Not available"}
                    />
                    <RecordField
                      label="Samples / Services"
                      value={sampleSummary}
                    />
                    <RecordField label="Frequency" value={frequency} />
                    ...
                    {location.email && (
                      <RecordField label="Email" value={location.email} />
                    )}
                  </div>
                </article>
              );
                    ...
                    <div className="company360-items-wrap">
                      ...
                      <div className="company360-table-wrap">
                        ...
                        <table className="company360-table">
                          ...
                          <tbody>{items.map((item) => <tr key={item.id}><td>{item.service}</td><td>{item.description || "—"}</td><td>{item.quantity}</td><td>{formatCurrency(item.unitPrice)}</td><td>{formatCurrency(item.amount)}</td></tr>)}</tbody>
              ...
              return (
                ...
                <article className="company360-record-card" key={workOrder.id}>
                  ...
                  <div className="company360-record-body">
                    ...
                    <RecordField label="Service Window" value={workOrder.expectedStart || workOrder.expectedEnd ? `${formatDate(workOrder.expectedStart)} → ${formatDate(workOrder.expectedEnd)}` : "Not scheduled"} />
          ...
          <div className="company360-card-grid two">
            ...
            {samples.map((sample) => (
              ...
              <article className="company360-record-card" key={sample.id}>
                ...
                <RecordHead icon={<TestTube2 size={18} />} title={sample.sampleNumber} subtitle={`${sample.sampleType} · ${sample.sampleCount} ${sample.sampleCount === 1 ? "sample" : "samples"}`} status={sample.status} />
          ...
          <>
            ...
            <div className="company360-table-wrap company360-payment-table">
              ...
              <table className="company360-table">
                ...
                <tbody>{payments.map((payment) => <tr key={payment.id}><td>{formatDate(payment.paymentDate)}</td><td><strong>{formatCurrency(payment.amount)}</strong></td><td>{payment.paymentMethod || "—"}</td><td>{payment.reference || "—"}</td><td><span className={`company360-inline-status ${statusClass(payment.status)}`}>{payment.status}</span></td></tr>)}</tbody>
...
<Section eyebrow="360° History" title="Recent Client Timeline" description="Latest activity across engagement, quotations, orders, samples, reports and payments." icon={<PackageCheck size={19} />}>
